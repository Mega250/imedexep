...
@router.get("/{patient_id}/socioeconomic", response_model=SocioeconomicData)
async def get_socioeconomic(
    ...
    return await PatientService(session).get_socioeconomic(patient_id, caller=token)


@router.patch("/{patient_id}/socioeconomic", response_model=SocioeconomicData)
async def update_socioeconomic(
    patient_id: int,
    body: SocioeconomicData,
    token: TokenPayload = Depends(
        require_roles("doctor", "institution_admin", "superadmin")
    ),
    session: AsyncSession = Depends(get_rls_session),
):
    return await PatientService(session).update_socioeconomic(patient_id, body, caller=token)
