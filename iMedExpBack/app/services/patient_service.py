...
class PatientService:
    ...
    async def get_socioeconomic(
        ...
        if caller is not None:
            await ensure_patient_ownership(self.session, caller, patient_id)
        data = await self.repo.get_socioeconomic(patient_id)
        return SocioeconomicData(**data)
    ...
    async def update_socioeconomic(
        ...
        await self.repo.upsert_socioeconomic(patient_id, values)
        updated = await self.repo.get_socioeconomic(patient_id)
        return SocioeconomicData(**updated)
