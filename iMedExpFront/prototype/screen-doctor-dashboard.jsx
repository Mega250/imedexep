...
// ─────────────────────────────────────────────────────────────
// STATS TICKER — top, compact, horizontal
// ─────────────────────────────────────────────────────────────
function StatsTicker() {
  const items = [
    ['42',    'consultas',   'esta semana',     '+12%'],
    ['178',   'pacientes',   'este mes',        '+8 nuevos'],
    ['94%',   'adherencia',  'tus crónicos',    '↑ 6 pts'],
    ['12 min','tiempo medio','por consulta',    '−2 min'],
  ];
  return (
    <div style={{
      margin: '12px 48px 0',
      ...
      {items.map(([n, label, ctx, trend], i) => (
        <div key={i} style={{
          padding: '16px 22px',
          borderRight: i < 3 ? '1px solid var(--rule-2)' : 0,
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 12.5, color: 'var(--ink)', lineHeight: 1.2 }}>
              <span style={{ fontWeight: 500 }}>{label}</span>{' '}
              <span style={{ color: 'var(--ink-3)' }}>· {ctx}</span>
            </div>
            <div className="mono" style={{ fontSize: 10.5, color: 'var(--ok)', marginTop: 4, letterSpacing: '0.04em' }}>
              {trend}
            </div>
          </div>
        </div>
...
// ─────────────────────────────────────────────────────────────
// ACCESO RÁPIDO — touch-friendly tiles
// ─────────────────────────────────────────────────────────────
function AccesoRapido() {
  const tiles = [
    ['search',  'Buscar paciente',  '⌘K',          false],
    ['plus',    'Nueva nota',        'En consulta',  false],
    ['rx',      'Receta digital',    'Firma activa', false],
    ['qr',      'Vincular paciente', 'Generar QR',   true],
    ['cal',     'Agendar cita',      'Nueva',        false],
    ['study',   'Solicitar estudio', 'Lab / imagen', false],
  ];
  return (
    <div style={{ margin: '14px 48px 0' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
        <span className="eyebrow">Acceso rápido</span>
        <span className="mono" style={{ fontSize: 11, color: 'var(--ink-3)' }}>optimizado para iPad / móvil</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 10 }}>
        {tiles.map(([icon, label, hint, accent], i) => (
          <button key={i} style={{
          ...
          onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 14px 30px -12px rgba(3,4,94,0.18)'; }}
          onMouseLeave={(e) => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = 'none'; }}
          >
            <span style={{
              width: 34, height: 34, borderRadius: 8,
              background: accent ? 'rgba(255,255,255,0.18)' : 'var(--paper-3)',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <div className="mono" style={{
                fontSize: 10.5,
                color: accent ? 'rgba(255,255,255,0.78)' : 'var(--ink-3)',
  ...
  return (
    <div className="imx" style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <NextPatientHero />
        <AccesoRapido />
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 14, padding: '20px 48px 24px' }}>
          <AgendaToday />
          <PendientesCard />
        </div>
      </div>
