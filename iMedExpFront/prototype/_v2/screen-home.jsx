...
const Pulse = ({ color = 'var(--accent)' }) => (
  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
    <span style={{
      ...
      background: color, boxShadow: `0 0 0 4px ${color === 'var(--accent)' ? 'rgba(0,150,199,0.22)' : 'rgba(255,255,255,0.18)'}`,
    }} />
  </span>
);
  ...
  return (
    <section style={{ padding: '32px 56px 96px' }}>
      <div style={{
        ...
        boxShadow: '0 2px 0 var(--rule-2), 0 30px 60px -30px rgba(3,4,94,0.18)',
      }}>
        {/* window chrome */}
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', minHeight: 520 }}>
          ...
          <div style={{ borderRight: '1px solid var(--rule)', padding: '20px 18px' }}>
            ...
            {[
              ...
              <div key={i} style={{
                ...
                gap: 12, padding: '10px 10px', borderRadius: 'var(--r-md)',
          ...
          <div style={{ padding: '24px 28px' }}>
            ...
            <div style={{
              ...
              borderRadius: 'var(--r-lg)', display: 'flex', alignItems: 'center', gap: 14,
            }}>
              <span style={{
                fontFamily: 'var(--mono)', fontSize: 11, letterSpacing: '0.12em',
                padding: '4px 9px 5px', background: 'var(--alert)', color: 'var(--white)', borderRadius: 999,
              }}>ALERGIA SEVERA</span>
            ...
            {/* 2x2 expediente */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, marginTop: 22, border: '1px solid var(--rule)', borderRadius: 'var(--r-lg)', overflow: 'hidden', background: 'var(--paper-2)' }}>
              {[
                {
                ...
                <div key={i} style={{
                  padding: '16px 18px',
                  background: i === 0 ? 'var(--white)' : i === 3 ? 'var(--white)' : 'var(--paper-2)',
...
// Roles
// ─────────────────────────────────────────────────────────────
function RoleCard({ tag, title, who, body, ctas, tint }) {
  const palette = {
    light: { bg: 'var(--white)', fg: 'var(--ink)', sub: 'var(--ink-2)', chip: 'chip accent' },
    mid:   { bg: 'var(--paper-3)', fg: 'var(--ink)', sub: 'var(--ink-2)', chip: 'chip accent' },
    dark:  { bg: 'var(--ink)', fg: 'var(--paper)', sub: 'rgba(255,255,255,0.72)', chip: 'chip dark' },
  }[tint || 'light'];
  return (
    <div style={{
      display: 'flex', flexDirection: 'column', gap: 14,
      padding: '28px 28px 24px',
      border: tint === 'dark' ? '1px solid var(--ink)' : '1px solid var(--rule)',
      background: palette.bg, color: palette.fg, borderRadius: 'var(--r-xl)',
      boxShadow: tint === 'dark' ? '0 30px 60px -30px rgba(3,4,94,0.4)' : '0 2px 0 var(--rule-2)',
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span className={palette.chip}>{tag}</span>
        <span className="mono" style={{ fontSize: 11, color: palette.sub }}>{who}</span>
      </div>
      <h3 style={{ fontFamily: 'var(--serif)', fontSize: 34, lineHeight: 1, fontWeight: 400, letterSpacing: '-0.02em' }}>{title}</h3>
      <p style={{ fontSize: 14.5, lineHeight: 1.5, color: palette.sub }}>{body}</p>
      <div style={{ flex: 1 }} />
      <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
        {ctas.map((c, i) => (
          <button key={i} className={`btn sm ${i === 0 ? (tint === 'dark' ? 'invert' : '') : (tint === 'dark' ? 'dark-ghost' : 'ghost')}`}>{c}</button>
        ))}
      </div>
    </div>
  );
...
function HomeAnti() {
  return (
    <section style={{ padding: '32px 56px 96px' }}>
      <div style={{ border: '1px solid var(--rule)', background: 'var(--paper-2)', borderRadius: 'var(--r-xl)', overflow: 'hidden' }}>
