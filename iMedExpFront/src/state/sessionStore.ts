...
import { CurrentUser, getCurrentUser, refreshTokens, TokenResponse } from "@/services/auth/authApi";
...
export async function refreshCurrentSession(): Promise<void> {
  ...
  try {
    ...
    const body = isWeb ? {} : { refresh_token: session.tokens.refresh_token };
    const newTokens = await refreshTokens(body);
    const merged = {
      ...newTokens,
      refresh_token: newTokens.refresh_token || session.tokens.refresh_token
    };
    await saveSession(merged, session.user);
  } catch {
    // best-effort: si falla el refresh, se sigue con el token actual
  }
}
