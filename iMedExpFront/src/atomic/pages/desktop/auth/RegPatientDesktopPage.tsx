...
import { registerPatient } from "@/services/auth/authApi";
import {
...
export function RegPatientDesktopPage() {
  ...
  function validateCurrentStep(targetStep = step): string | null {
    ...
    if (targetStep === 1) {
      return (
        validatePhoneMx(form.phone) ??
        validatePostalCodeMx(form.postalCode) ??
        (!form.street.trim() ? "Calle y número es obligatorio." : null) ??
        (!form.neighborhood.trim() ? "Colonia o comunidad es obligatoria." : null) ??
        (!form.city.trim() ? "Municipio o ciudad es obligatorio." : null) ??
        (!form.state.trim() ? "Estado es obligatorio." : null)
      );
    }
    return null;
  }
      ...
      <View style={styles.content}>
        ...
        <FadeIn style={styles.form}>
          ...
          <Text style={styles.h2}>
            {step === 0 ? "Datos personales." : step === 1 ? "Dirección." : "Historia inicial."}
          </Text>
          <Text style={styles.lead}>
            {step === 0
              ? "Estos datos aparecerán en tu expediente y se reanudan si sales antes de terminar."
              : step === 1
              ? "Guarda el domicilio actual. El código postal queda listo para conectarse a un proveedor de colonias."
