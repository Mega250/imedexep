...
import { fetchPatientFull, fetchPatientsList, PatientFull } from "@/services/api/patientsApi";
...
function TabContent({ tab, patient }: { tab: string; patient: PatientFull }) {
  if (tab === "Vitales") {
    return (
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (selectedId !== null) {
          try {
            const selectedPatient = await fetchPatientFull(selectedId);
            if (cancelled) {
              return;
            }
        ...
        const p = await fetchPatientFull(list.items[0].id);
        if (cancelled) {
          return;
        }
        setPatient(p);
    ...
    >
      {loading ? (
        ...
        <>
          ...
          <FadeIn delay={120}>
            <TabContent tab={activeTab} patient={patient} />
          </FadeIn>
        </>
      )}
    </DesktopShell>
